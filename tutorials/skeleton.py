...
cp_index = create_controlled_const_profile(
    prosumer, input_params, result_params, time_series_input_bhp, period)
bhp_index = create_controlled_booster_heat_pump(prosumer, bhp_type, bhp_name, level = 1 )

ice_chp_index = create_controlled_ice_chp(prosumer, size_kw, fuel, altitude_m, name, level=1)

heat_storage_index = create_controlled_heat_storage(prosumer, q_capacity_kwh,level = 1,order=1)

heat_demand_index = create_controlled_heat_demand(prosumer, scaling=1.0, level = 1, order =1)



GenericMapping(prosumer,
                   initiator_id=cp_index,
                   initiator_column=["t_source_cp_k", "mode_cp"],
                   responder_id=bhp_index,
                   responder_column=["t_source_k", "mode"],
)

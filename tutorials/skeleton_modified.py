...
cp_index = create_controlled_const_profile(
    bhp_prosumer, input_params, result_params, period=period, data_source=time_series_input_bhp)
...
for i in combinations:
    ...
    time_series_input_chiller.df['Cooling demand Q_load [kJ/h]'] = chill_scaling * (og_time_series_input_chiller / 3600)
    ...
    E_balance = chp_p_el_out_kw - bhp_p_el_in_kw - chiller_p_el_in_kw
    E_target = E_balance - flex_t

    deltaE = -flex_t

    rho = (chp_p_el_out_kw / chp_p_th_out_kw).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    COP = (Q_bhp_out / bhp_p_el_in_kw).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    EER = res_chiller['eer']

    downE = (-deltaE).clip(lower=0.0)
    upE = (deltaE).clip(lower=0.0)

    #chosen arbitrary, how to split shares?
    share_cool = 0.3

    dQ_ch = (share_cool * downE * EER).fillna(0.0)
    dQ_bhp = ((1.0 - share_cool) * downE * COP).fillna(0.0)

    rho = rho.replace(0.0, np.nan)
    dQ_chp = (upE / rho).fillna(0.0)
    #limit the change of deltaQ
    cap = 1
    dQ_bhp = dQ_bhp.clip(upper=cap * og_time_series_input_bhp)
    dQ_chp = dQ_chp.clip(upper=cap * og_time_series_input_chp)
    dQ_ch = dQ_ch.clip(upper=cap * (og_time_series_input_chiller / 3600.0))
    #changing demand and adding the deltQ at next timestep?
    shift = 0
    ...
    E_new = (P_chp_el_new - P_bhp_in_new - P_ch_in_new)

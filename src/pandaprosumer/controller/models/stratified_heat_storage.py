...
class StratifiedHeatStorageController(BasicProsumerController):
    ...
    def q_to_receive_kw(self, prosumer):
        ...
        self.q_received_kw = self.t_m_to_q_kw(mdot_required_kg_per_s, t_required_in_c, t_required_out_c)
        return self.q_received_kw
    ...
    def _calculate_heat_storage(self, prosumer, mdot_demand_kg_per_s, t_received_in_c, t_demand_out_c, t_demand_in_c,
        ...
        t_required_in_c, t_required_out_c, mdot_required_kg_per_s = self._t_m_to_receive_init(prosumer)
        if np.isnan(t_received_in_c) and np.isnan(mdot_received_kg_per_s): # in case of Generic Mapping
            t_received_in_c = t_required_in_c + 20  # Received Temperature set to the required Temperature Todo: How to set the temperature
            ...
            mdot_received_kg_per_s = q_received_kw * 1e3 / (cp_received_j_per_kgk * (t_received_in_c - t_required_out_c))

            q_demand_kw = self.q_requested_kw(prosumer)
            ...
            rho_mean_kg_per_m3 = self.fluid.get_density(CELSIUS_TO_K + np.mean(self._layer_temps_c))
            if q_received_kw > q_demand_kw:
                q_bypass_kw = q_demand_kw
                q_charge_requested_kw = q_received_kw - q_demand_kw
                mdot_charge_requested_kg_per_s = q_charge_requested_kw * 1e3 / (
                            cp_received_j_per_kgk * (t_received_in_c - t_required_out_c))
                q_delivered_kw = q_demand_kw
                q_discharge_kw = 0
                mdot_discharge_kg_per_s = 0
                m_layer_kg = self.A_m2 * self.dz_m * rho_mean_kg_per_m3
                nb_cold_layers = np.sum(np.array(self._layer_temps_c) < t_required_in_c - 1.5)  # Added tolerance
                mdot_charge_max_kg_per_s = nb_cold_layers * m_layer_kg / self.resol
                mdot_charge_kg_per_s = min(mdot_charge_requested_kg_per_s, mdot_charge_max_kg_per_s)
            else:
                mdot_charge_kg_per_s = 0
                q_bypass_kw = q_received_kw
                m_layer_kg = self.A_m2 * self.dz_m * rho_mean_kg_per_m3
                nb_warm_layers = np.sum(np.array(self._layer_temps_c) > t_demand_out_c - .1)  # Added tolerance
                mdot_discharge_kg_per_s_max = nb_warm_layers * m_layer_kg / self.resol
                q_discharge_requested_kw = q_demand_kw - q_bypass_kw
                mdot_discharge_requested_kg_per_s = q_discharge_requested_kw * 1e3 / (cp_discharge_j_per_kgk * (t_demand_out_c - t_demand_in_c))
                mdot_discharge_kg_per_s = min(mdot_discharge_kg_per_s_max, mdot_discharge_requested_kg_per_s)
                q_discharge_kw = mdot_discharge_kg_per_s * (cp_discharge_j_per_kgk * (t_demand_out_c - t_demand_in_c)) / 1e3
                q_delivered_kw = q_discharge_kw + q_bypass_kw

            mdot_bypass_kg_per_s = q_bypass_kw * 1e3 / (cp_received_j_per_kgk * (t_received_in_c - t_required_out_c))
            ...
            q_discharge_kw = mdot_discharge_kg_per_s * cp_discharge_j_per_kgk * (
                        t_discharge_out_c - t_demand_in_c) / 1e3
...
class StratifiedHeatStorageController(BasicProsumerController):
    ...
    def _get_max_stored_energy_kwh(self, extraction_temp_c, t_charge_c):
        ...
        ret = sum([float(self.fluid.get_density(CELSIUS_TO_K + t_charge_c)) * self.A_m2 *
                   float(self.fluid.get_heat_capacity(CELSIUS_TO_K + t_charge_c)) *
                   (t_charge_c - self.init_layer_temps_c[z]) * self.dz_m
                   for z in range(self.N_l) if t_charge_c >= extraction_temp_c - 1.5]) / 3.6e6
        return ret
    ...
    def _get_stored_energy_kwh(self, t_extraction_c):
        ...
        ret = sum([float(self.fluid.get_density(CELSIUS_TO_K + self._layer_temps_c[z])) * self.A_m2 *
                   float(self.fluid.get_heat_capacity(CELSIUS_TO_K + self._layer_temps_c[z])) *
                   (self._layer_temps_c[z] - self.init_layer_temps_c[z]) * self.dz_m
                   for z in range(self.N_l) if self._layer_temps_c[z] >= t_extraction_c - 1.5]) / 3.6e6
        return ret

    def _calculate_heat_storage(self, prosumer, mdot_demand_kg_per_s, t_received_in_c, t_demand_out_c, t_demand_in_c,
                                t_discharge_out_c, mdot_received_kg_per_s, t_charge_out_c):
        e_stored = self._get_stored_energy_kwh(t_demand_out_c)
        e_max_stored = self._get_max_stored_energy_kwh(t_demand_out_c, t_received_in_c)

        soc = e_stored / e_max_stored
        print(mdot_received_kg_per_s)
        t_required_in_c, t_required_out_c, mdot_required_kg_per_s =  self._t_m_to_receive_init(prosumer)
        cp = 4180

        q_received_kw = mdot_received_kg_per_s * cp * (t_received_in_c - t_required_out_c) / 1e3
        q_demand_kw = mdot_demand_kg_per_s * cp * (t_demand_out_c - t_demand_in_c) / 1e3
        ...
        if not self.bypass:
            ...
            if mdot_toprovide_kg_per_s > mdot_received_kg_per_s:
                ...
                if mdot_discharge_kg_per_s < 0:
                    ...
                    #e_bypass = mdot_bypass_kg_per_s * (t_received_in_c - t_demand_out_c)
                    #mdot_discharge_kg_per_s = (e_demand - e_bypass) / (t_discharge_out_c - t_demand_in_c)
                    mdot_discharge_kg_per_s = (q_demand_kw - q_received_kw) * 1e3 / (cp * (t_discharge_out_c - t_demand_in_c))
            else:
                # If this energy can be provided, charging with the extra mass flow
                mdot_bypass_kg_per_s = mdot_toprovide_kg_per_s
                mdot_charge_kg_per_s = mdot_received_kg_per_s - mdot_bypass_kg_per_s
        ...
        #q_discharge_kw = mdot_discharge_kg_per_s * cp_discharge_j_per_kgk * (t_discharge_out_c - t_demand_out_c) / 1e3
        ...
        return (q_delivered_kw, q_bypass_kw, q_discharge_kw, e_stored_kwh,
             ...
             mdot_delivered_kg_per_s, t_demand_in_c, t_delivered_out_c,
             mdot_charge_kg_per_s, t_charge_out_c,
             mdot_discharge_kg_per_s, t_discharge_out_c, soc)
    ...
    def control_step(self, prosumer):
        ...
        while rerun:
            ...
            (q_delivered_kw, q_bypass_kw, q_discharge_kw, e_stored_kwh,
             ...
             mdot_delivered_kg_per_s, t_demand_in_c, t_delivered_out_c,
             mdot_charge_kg_per_s, t_charge_out_c,
             mdot_discharge_kg_per_s, t_discharge_out_c, soc) = self._calculate_heat_storage(prosumer,
                                                                                        mdot_demand_kg_per_s,
                                                                                        self._t_received_in_c,
                                                                                        t_demand_out_c,
                                                                                        t_demand_in_c,

...
class NetworkCoupling(MappedController):
    """
    NetworkCoupling
    ...
    def control_step(self, net):
        ...
        net[self.element_name].loc[self.element_index, self.input_columns] = self.inputs
        ...
        if self.temp_fluid_map_input_col is not None and self.mdot_fluid_map_input_col is not None:
            if not np.isnan(self.input_mass_flow_with_temp[FluidMixMapping.TEMPERATURE_KEY]) and not np.isnan(self.input_mass_flow_with_temp[FluidMixMapping.MASS_FLOW_KEY]):
                net[self.element_name].loc[self.element_index, self.temp_fluid_map_input_col] = self.input_mass_flow_with_temp[FluidMixMapping.TEMPERATURE_KEY] + CELSIUS_TO_K
                net[self.element_name].loc[self.element_index, self.mdot_fluid_map_input_col] = self.input_mass_flow_with_temp[FluidMixMapping.MASS_FLOW_KEY]

        # EnergySystem FluidMix mapping outputs to prosumers
        if self.temp_fluid_map_output_idx is not None and self.mdot_fluid_map_output_idx is not None:
            result_fluid_mix = [{FluidMixMapping.TEMPERATURE_KEY: results[0][self.temp_fluid_map_output_idx] - CELSIUS_TO_K,
...
def create_controlled_network_coupling(net,
                                        ...
                                        result_columns,
                                       level):
    load_ctrl_object = NetworkCouplingData(
        element_name=element_name,
        element_index=[element_index],
        input_columns= input_columns,
        result_columns=result_columns,
    )
    controller = NetworkCoupling(
        net=net,
        load_ctrl_object=load_ctrl_object,
        # index=load_ctrl_object.element_index[0],
        level = level
    )
    return controller